 ...
 *
 */
public class CornerBasedCohesionGraphTest {
	...
	public CornerBasedCohesionGraphTest(int dimension, int numPoints, int numNeighbors) {
		this.d = dimension;
		this.n = numPoints;
		this.k = numNeighbors;
	...
	public static void main(String[] args) {
		...
		// Random g = new Random();
		CornerBasedCohesionGraphTest test = new CornerBasedCohesionGraphTest(d, n, k);
	...
	/*
	 * return codes: 0 if x, y in same template, reported in same component (TP). 1 if
	 * x, y in same template, reported in different components (FN). 2 if x, y in
	...
	ToIntBiFunction<PointInSimplex, PointInSimplex> clusterValidator = (x, y) -> {
		boolean sameBooster = (Integer.compare(x.getTemplate(), y.getTemplate()) == 0); // fields of x, y
	...
	private void reportClusterQuality() {
		...
		System.out.println("Out of " + numSameCorner + " in the same template, " + falseNegativeRate
